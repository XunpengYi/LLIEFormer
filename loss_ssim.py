...
def ssim(img1, img2, window_size=24, window=None, size_average=True, full=False, val_range=None):
...
def ssim_loss(img1,img2):
    ...
    try:
        ...
        assert B1 == B2, \
            f"Model input pair doesn't match, cause error in loss."
    except:
        C1, H1, W1 = img1.shape
        C2, H2, W2 = img2.shape
        ...
        B1 = 1
        B2 = 1
        loss = ssim_compute(img1,img2)
        return loss
    loss = 0

    for index in range(B1):
        img1b=img1[index].unsqueeze(0)
        img2b=img2[index].unsqueeze(0)
        loss = loss + ssim_compute(img1b,img2b)
